...
public class Rodovia {
	...
	public static Rodovia cadastrar() {
		...
		String periculosidade = JOptionPane.showInputDialog(
				"Qual a perículosidade da rodovia que deseja cadastrar?\nDigite Baixo, Médio ou Alta: ");
	...
	public static Rodovia listarRodoviaMaisFatal(List<Acidente> acidentes) {
		...
		for (Acidente acidente : acidentes) {
			List<Acidente> acidentesFiltrados = acidentes.stream().filter(acidenteAux -> acidenteAux.getRodovia()
					.getNome().toUpperCase().equals(acidente.getRodovia().getNome().toUpperCase()))
					.collect(Collectors.toList());

			double totalAcidentes = acidentesFiltrados.stream()
					.mapToDouble(acidenteAux -> acidenteAux.getQtVitimasFatais()).sum();
